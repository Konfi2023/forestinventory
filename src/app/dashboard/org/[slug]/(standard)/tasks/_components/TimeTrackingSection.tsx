...
const CATEGORY_KEYS: Record<string, string> = {
  MANUAL_WORK:  'categoryManualWork',
  MACHINE_WORK: 'categoryMachineWork',
  PLANNING:     'categoryPlanning',
  TRAVEL:       'categoryTravel',
  INSPECTION:   'categoryInspection',
};

function fmtMins(mins: number | null | undefined, noEstLabel: string): string {
  if (!mins) return noEstLabel;
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] uppercase font-bold text-slate-500 tracking-wide">{t("timeTrackingLabel")}</span>
        </div>
        <Button size="sm" variant="ghost"
          className="h-7 text-xs text-emerald-400 hover:text-emerald-300 hover:bg-white/5 px-2"
          onClick={() => setShowForm(v => !v)}>
          <Plus size={13} className="mr-1" /> {t("addEntry")}
        </Button>
      </div>

      <div className="bg-white/5 rounded-lg p-3 space-y-2 border border-white/5">
        <div className="flex justify-between text-sm">
          <span className="text-slate-400">{t("estimated")}</span>
          <span className="text-white font-mono">{fmtMins(estimatedTime, t("noEstimate"))}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-slate-400">{t("booked")}</span>
        ...
        {progress !== null && (
          <div className="space-y-1">
            ...
            {isOver && estimatedTime && (
              <p className="text-[11px] text-red-400">{t("overEstimate", { time: fmtMins(totalActual - estimatedTime, t("noEstimate")) })}</p>
            )}
          </div>
        )}
      </div>
      ...
      {showForm && (
        <div className="bg-white/5 border border-white/10 rounded-lg p-3 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">{t("date")}</label>
              <Input type="date" value={date} onChange={e => setDate(e.target.value)}
                className="h-8 bg-black/30 border-white/10 text-white text-sm" />
            </div>
            <div>
              <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">{t("hoursLabel")}</label>
              <Input type="number" min="0.25" step="0.25" placeholder={t("hoursPlaceholder")}
                value={hours} onChange={e => setHours(e.target.value)}
                className="h-8 bg-black/30 border-white/10 text-white text-sm" />
            </div>
          </div>
          <div>
            <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">{t("category")}</label>
            <select value={category} onChange={e => setCategory(e.target.value)}
              className="w-full h-8 bg-black/30 border border-white/10 rounded-md text-white text-sm px-2">
              {Object.entries(CATEGORY_KEYS).map(([k, v]) => <option key={k} value={k}>{t(v)}</option>)}
            </select>
          </div>
          <div>
            <label className="text-[10px] text-slate-500 uppercase font-bold block mb-1">{t("noteOptional")}</label>
          ...
          <div className="flex gap-2 pt-1">
            <Button size="sm" onClick={handleLog} disabled={saving}
              ...
              {saving ? <Loader2 size={13} className="animate-spin" /> : t('book')}
            </Button>
            <Button size="sm" variant="ghost" onClick={() => setShowForm(false)}
              className="text-slate-400 h-8 text-xs">{t('cancel')}</Button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-1">
          {entries.map((e: any) => (
            <div key={e.id} className="flex items-center gap-2 text-xs text-slate-400 group px-1">
              ...
              <span className="text-slate-600 shrink-0">{CATEGORY_KEYS[e.category] ? t(CATEGORY_KEYS[e.category]) : e.category}</span>
      ...
      {entries.length === 0 && !showForm && (
        <p className="text-xs text-slate-600 text-center py-2">{t("noTimeEntries")}</p>
      )}
    </div>
  );
}
