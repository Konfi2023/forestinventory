...
export default async function OrgDashboardPage({
  ...
  const quickLinks = [
    { href: `/dashboard/org/${slug}/map`,        icon: Map,          label: t('linkMap'),        color: 'text-blue-600 bg-blue-50' },
    { href: `/dashboard/org/${slug}/tasks`,      icon: ClipboardList, label: t('linkTasks'),      color: 'text-amber-600 bg-amber-50' },
    { href: `/dashboard/org/${slug}/operations`, icon: PackageOpen,   label: t('linkOperations'), color: 'text-orange-600 bg-orange-50' },
    { href: `/dashboard/org/${slug}/calendar`,   icon: CalendarDays,  label: t('linkCalendar'),   color: 'text-violet-600 bg-violet-50' },
    { href: `/dashboard/org/${slug}/biomass`,    icon: Leaf,          label: t('linkBiomass'),    color: 'text-green-600 bg-green-50' },
    { href: `/dashboard/org/${slug}/forest`,     icon: Trees,         label: t('linkForests'),    color: 'text-emerald-600 bg-emerald-50' },
  ];
  ...
  return (
    <div className="overflow-y-auto h-full bg-slate-50">
      <div className="p-4 md:p-8 max-w-[1600px] mx-auto space-y-6">
        ...
        {/* Header */}
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900">{t('welcomeBack')}</h2>
        ...
        <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
          <div className="bg-white border border-slate-200 rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">{t('kpiTotalArea')}</p>
              <Trees size={16} className="text-green-600" />
            </div>
            <p className="text-2xl font-bold text-slate-900">{totalHa > 0 ? `${totalHa.toFixed(0)} ha` : '—'}</p>
            <p className="text-xs text-slate-400 mt-1">{t('forestAreas', { count: accessible.length })}</p>
          </div>

          <div className="bg-white border border-slate-200 rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">{t('kpiTeam')}</p>
              <Users size={16} className="text-blue-500" />
            </div>
            <p className="text-2xl font-bold text-slate-900">{memberCount}</p>
            <p className="text-xs text-slate-400 mt-1">{t('activeMembers')}</p>
          </div>

          <div className="bg-white border border-slate-200 rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">{t('kpiOpenTasks')}</p>
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5">
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-medium text-slate-500 uppercase tracking-wide">{t('kpiMyTasks')}</p>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-semibold text-slate-800 text-sm">{t('myTasks')}</h3>
                {myTasks.length > 0 && (
              ...
              <Link href={`/dashboard/org/${slug}/tasks`}
                ...
                {t('showAll')} <ChevronRight size={13} />
              </Link>
            </div>
                    ...
                    <Link key={task.id}
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span className={`text-xs font-semibold px-2 py-0.5 rounded border ${PRIORITY_COLOR[task.priority] ?? PRIORITY_COLOR.MEDIUM}`}>
                          {PRIORITY_LABEL[task.priority] ?? t('priorityNormal')}
                        </span>
                        {task.dueDate && (
          ...
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <div className="px-5 py-3.5 border-b border-slate-100">
              <h3 className="font-semibold text-slate-800 text-sm">{t('quickAccess')}</h3>
            </div>
        ...
        {ackHistory.length > 0 && (
          <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-5 py-3.5 border-b border-slate-100">
              ...
              <h3 className="font-semibold text-slate-800 text-sm">{t('warningHistory')}</h3>
              <span className="text-xs text-slate-400 ml-1">({t('acknowledgedAlarms')})</span>
            </div>
            <div className="divide-y divide-slate-50">
              {ackHistory.map(ack => (
                <div key={ack.id} className="flex items-center gap-3 px-5 py-2.5 hover:bg-slate-50">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-sm font-medium text-slate-700">
                      {ack.alertType === 'STORM' ? t('stormWarning') : t('forestDamageWarning')}
                    </span>
                    ...
                    {ack.isTest && (
                      <span className="ml-2 inline-flex items-center gap-0.5 text-xs text-violet-600">
                        <FlaskConical size={10} /> {t('testLabel')}
                      </span>
                    )}
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-xs text-slate-400">
                      {ack.dismissedAt.toLocaleDateString(locale, { day: '2-digit', month: 'short', year: 'numeric' })}
                    </p>
