...
function defaultT(key: string) { return key; }

export function BhdMeasurement({ photoSrc, onMeasured, onSkip, t: _t }: Props) {
  const i = _t ?? defaultT;
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black flex flex-col">
      ...
      <div className="shrink-0 flex items-center justify-between px-4 py-3">
        <div>
          <h3 className="text-white text-sm font-semibold">{i('bhdMeasure')}</h3>
          <p className="text-slate-500 text-xs">
            {mode === 'detecting' && i('searchingCard')}
            {mode === 'auto' && phase === 'trunk' && i('slideTrunkEdges')}
            {mode === 'manual' && phase === 'card' && i('slideCardEdges')}
            {mode === 'manual' && phase === 'trunk' && i('slideTrunkEdges')}
            {isResult && `${i('bhd')} ${bhdValue} cm`}
          </p>
        </div>
      ...
      <div className="shrink-0 px-4 py-2 text-center min-h-[48px]">
        {mode === 'detecting' && (
          <div className="flex items-center justify-center gap-2 text-slate-400 text-sm">
            <Loader2 size={14} className="animate-spin" /> {i('searchingCard')}
          </div>
        )}
        ...
        {isResult && (
          <div className="text-emerald-400">
            ...
            <p className="text-xs text-emerald-500 mt-0.5">
              {i('bhdMeasured')} {mode === 'auto' ? i('viaCard') : i('viaCreditCard')}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="shrink-0 px-4 py-3 space-y-2">
        {isResult && mode === 'manual' && (
          <button onClick={toggleCardSide}
            ...
            {cardMm === CARD_LONG_MM ? i('refLong') : i('refShort')} — {i('refTapToSwitch')}
          </button>
        )}
          ...
          {!isResult && (
            <>
              ...
              <button onClick={confirmPhase}
                ...
                {phase === 'card' ? i('cardConfirmed') : i('calculateBhd')}
              </button>
            </>
          )}
          {isResult && (
