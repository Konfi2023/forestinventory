  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-slate-950/90 backdrop-blur-xl border-b border-white/[0.06] shadow-lg shadow-black/10'
          : 'bg-transparent'
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center shrink-0">
          <svg height="30" viewBox="0 0 285 47" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ height: 30, width: 'auto' }}>
            ...
            <text x="148" y="33" fill="#4ade80" fontFamily="sans-serif" fontWeight="bold" fontSize="26" letterSpacing="-0.5">Manager</text>
          </svg>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          <button
            onClick={() => signIn('keycloak')}
            className="text-sm font-medium bg-green-600 hover:bg-green-500 text-white px-5 py-2 rounded-xl transition-colors"
          >
            Kostenlos starten
          </button>
        </div>
      ...
      {/* Mobile drawer */}
      {open && (
        <div className="md:hidden bg-slate-950/95 backdrop-blur-xl border-t border-white/[0.06] px-6 py-4 space-y-3">
          {NAV_LINKS.map(link => (
            <a
              key={link.href}
              href={link.href}
              onClick={() => setOpen(false)}
              className="block text-sm text-slate-400 hover:text-white py-1.5 transition-colors"
            >
              {link.label}
            </a>
          ))}
          <div className="pt-3 border-t border-white/[0.06] flex flex-col gap-2">
            <button onClick={() => signIn('keycloak')} className="text-sm text-slate-400 hover:text-white py-1.5 text-left">
              Einloggen
            </button>
            <button
              onClick={() => signIn('keycloak')}
              className="text-sm font-medium bg-green-600 text-white px-4 py-2.5 rounded-xl text-center"
            >
              Kostenlos starten
            </button>
          </div>
