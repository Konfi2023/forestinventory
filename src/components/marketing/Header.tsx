    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-sm">
      <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
        <Link href="/" className="flex items-center shrink-0">
          <svg height="24" viewBox="0 0 285 47" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ height: 24, width: 'auto' }}>
            <g>
        ...
        <div className="hidden md:flex items-center gap-4">
          <button onClick={() => signIn('keycloak')} className="text-[13px] text-stone-400 hover:text-stone-800 transition-colors">
            Einloggen
          </button>
          <button onClick={() => signIn('keycloak')} className="text-[13px] text-stone-800 hover:text-green-700 transition-colors">
            Kostenlos starten &rarr;
          </button>
        </div>

        <button className="md:hidden text-stone-400" onClick={() => setOpen(o => !o)} aria-label="Menü">
          {open ? <X size={20} /> : <Menu size={20} />}
        </button>
      </div>

      {open && (
        <div className="md:hidden bg-white px-6 py-4 space-y-3 border-t border-stone-100">
          {NAV_LINKS.map(link => (
            <a key={link.href} href={link.href} onClick={() => setOpen(false)} className="block text-sm text-stone-500 hover:text-stone-800 py-1">
              {link.label}
            </a>
          ))}
          <div className="pt-3 border-t border-stone-100 space-y-2">
            <button onClick={() => signIn('keycloak')} className="text-sm text-stone-500 block">Einloggen</button>
            <button onClick={() => signIn('keycloak')} className="text-sm text-stone-800 block">Kostenlos starten &rarr;</button>
          </div>
        </div>
      )}
    </header>
