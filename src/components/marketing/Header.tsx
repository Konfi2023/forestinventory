...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#0a0f0a]/80 backdrop-blur-md border-b border-white/5">
      <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
        <Link href="/" className="flex items-center shrink-0">
          <svg height="24" viewBox="0 0 285 47" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ height: 24, width: 'auto' }}>
            ...
            <text x="58" y="33" fill="#ffffff" fontFamily="sans-serif" fontWeight="700" fontSize="26" letterSpacing="-0.5">Forest</text>
            <text x="138" y="33" fill="#4ade80" fontFamily="sans-serif" fontWeight="700" fontSize="26" letterSpacing="-0.5">Manager</text>
          </svg>
        </Link>

        <nav className="hidden md:flex items-center gap-8">
          {NAV_LINKS.map(link => (
            <a key={link.href} href={link.href} className="text-[13px] text-slate-400 hover:text-white transition-colors">
              {link.label}
            </a>
          ))}
        </nav>
      ...
      {open && (
        <div className="md:hidden bg-[#0a0f0a] px-6 py-4 space-y-3 border-t border-white/10">
          {NAV_LINKS.map(link => (
            <a key={link.href} href={link.href} onClick={() => setOpen(false)} className="block text-sm text-slate-400 hover:text-white py-1">
              {link.label}
            </a>
          ))}
