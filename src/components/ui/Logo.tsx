...
export function Logo({ variant = "dark", height = 24, className }: LogoProps) {
  ...
  return (
    <svg
      ...
      className={className}
    >
      <mask id={id} style={{ maskType: "alpha" }} maskUnits="userSpaceOnUse" x="0" y="0" width="47" height="47">
        <rect width="47" height="47" fill="#D9D9D9" />
      </mask>
      <g mask={`url(#${id})`}>
        <path d={LEAF_PATH} fill={colors.leaf} />
      </g>
      <text x="58" y="33" fill={colors.forest} fontFamily="sans-serif" fontWeight="700" fontSize="26" letterSpacing="-0.5">Forest</text>
      <text x="138" y="33" fill={colors.manager} fontFamily="sans-serif" fontWeight="700" fontSize="26" letterSpacing="-0.5">Manager</text>
    </svg>
  );
}
