            ...
            <div class="np-auth-left">
                ...
                <form id="kc-form-login" class="np-auth-form" onsubmit="login.disabled = true; return true;" action="${url.loginAction}" method="post">
                    <div class="np-input-group">
                        ...
                        <input id="username" class="np-input" name="username" value="${(login.username!'')}" type="text" autofocus autocomplete="email" />
                    </div>

                    <div class="np-input-group">
                        <div class="np-input-label-pill-wrapper"><span class="np-input-label-pill">${msg("passwordLabel")}</span></div>
                        <input id="password" class="np-input" name="password" type="password" autocomplete="current-password" />
                    </div>
        ...
        if (canvas) {
          ...
          function draw() {
            ...
            for (var i = 0; i < nodes.length; i++) {
              for (var j = i + 1; j < nodes.length; j++) {
                ...
                if (dist < connectDist) {
                  ...
                  ctx.strokeStyle = 'rgba(45,61,42,' + alpha + ')';
                  ctx.lineWidth = 0.8;
                  ctx.beginPath();
            ...
            for (var k = 0; k < nodes.length; k++) {
              ...
              ctx.fillStyle = 'rgba(45,61,42,' + (p * 0.5) + ')';
              ctx.beginPath();
