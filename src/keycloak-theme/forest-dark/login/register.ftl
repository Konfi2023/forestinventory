...
<body>
    ...
    (function() {
      ...
      window.openExternalBrowser = function() {
        // Ueber NextAuth signin mit register=true → Keycloak kc_action=register
        var url = 'https://forest-manager.eu/signin?register=true';
      ...
      if (isApp) {
        ...
        if (authLeft) {
          authLeft.innerHTML = ''
            ...
            + '  <div class="app-brand" style="font-size:26px;font-weight:700;">Forest<span style="color:#2d3d2a;">Manager</span></div>'
            + '</div>'
            + '<div style="text-align:center;padding:0 20px;">'
            + '  <h1 style="font-size:22px;font-weight:700;color:#1a1e17;margin-bottom:16px;font-family:'Geist',sans-serif;">Willkommen bei ForestManager</h1>'
            + '  <p style="font-size:15px;color:#4a5148;line-height:1.7;margin-bottom:32px;">'
            ...
            + '     style="display:inline-block;padding:14px 36px;background:#2d3d2a;color:white;border-radius:14px;font-size:15px;font-weight:700;text-decoration:none;font-family:'Geist',sans-serif;">'
            + '    Auf forest-manager.eu registrieren'
            + '  </a>'
            + '  <div style="margin-top:24px;">'
            + '    <a href="' + document.querySelector('a.np-link[href*="login"]')?.href + '" style="font-size:14px;color:#2d3d2a;text-decoration:none;font-weight:600;">'
            + '      Zurück zum Login'
            + '    </a>'
            + '  </div>'
            + '</div>';
        ...
        if (canvas) {
          ...
          function draw() {
            ...
            for(var i=0;i<nodes.length;i++) for(var j=i+1;j<nodes.length;j++) {
              ...
              if(dist<200){ctx.strokeStyle='rgba(45,61,42,'+(1-dist/200)*0.25+')';ctx.lineWidth=0.8;ctx.beginPath();ctx.moveTo(nodes[i].x,nodes[i].y);ctx.lineTo(nodes[j].x,nodes[j].y);ctx.stroke();}
            }
            for(var k=0;k<nodes.length;k++){var n=nodes[k],p=Math.sin(t+n.pulse)*0.4+0.6;ctx.fillStyle='rgba(45,61,42,'+(p*0.5)+')';ctx.beginPath();ctx.arc(n.x,n.y,n.r*(0.8+p*0.4),0,Math.PI*2);ctx.fill();n.x+=n.vx;n.y+=n.vy;if(n.x<0||n.x>w)n.vx*=-1;if(n.y<0||n.y>h)n.vy*=-1;}
